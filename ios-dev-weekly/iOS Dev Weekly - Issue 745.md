# [Issue 745 – iOS Dev Weekly](https://iosdevweekly.com/issues/745/)

## Comment
- メールで紹介しているブログの一覧。RSSで取得できる
    - [iOS Dev Directory](https://iosdevdirectory.com/)
    - [opml形式](https://iosdevdirectory.com/opml/en/development.opml)も用意している

## [March 2026 - Hello - Apple Developer](https://developer.apple.com/hello/march26)
- Appleの今月のアップデートをハイライトできる

## Network Throttler
- ネットワークの速度を調整できるツール
    - https://macos-network-throttler.com/


## A Month With OpenAI’s Codex
- ChatGPTのCodexを利用して色々開発した話
- https://www.highcaffeinecontent.com/blog/20260301-A-Month-With-OpenAIs-Codex
- 必須スキルなんだと思わされる。AIによってできることの幅が広がるのである。

## Learning to develop more accessible iOS games
- ゲームアプリのアクセシビリティを考える
- 難しいことは必要がなく、基本的な知識の組み合わせと考慮次第である
- 思考の流れが参考になりそう

![](https://i.imgur.com/K7FB5DA.jpeg)

## Implementing Passkeys in iOS with AuthenticationServices

- [Implementing Passkeys in iOS with AuthenticationServices](https://tanaschita.com/ios-authentication-passkeys/)
- パスキーのおさらい
    - [パスキーのすべて ── 導入・UX設計・実装の紹介 / 20250213 パスキー開発者の集い - Speaker Deck](https://speakerdeck.com/kuralab/20250213-everything-about-passkey)
    - [公開鍵・秘密鍵とは？暗号化の仕組みをわかりやすく解説 | ITトレンド](https://it-trend.jp/encryption/article/64-0089)
